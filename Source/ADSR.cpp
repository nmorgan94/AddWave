...
void ADSR::setAttackRate(float rate) {
    /*!
     \param rate Sets attack rate
     */
    attackRate = rate;
    attackCoef = calcCoef(rate, targetRatioA);
    attackBase = (1.0 + targetRatioA) * (1.0 - attackCoef);
  //  //std::coutt << attackRate << " Attack Rate" << std::endl;
}

void ADSR::setDecayRate(float rate) {
    /*!
     \param rate Sets decay rate
     */
    decayRate = rate;
...
void ADSR::setReleaseRate(float rate) {
    /*!
     \param rate Sets release rate
     */
    releaseRate = rate;
    releaseCoef = calcCoef(rate, targetRatioDR);
    releaseBase = -targetRatioDR * (1.0 - releaseCoef);
 //   //std::coutt << releaseRate << std::endl;
}
...
void ADSR::setSustainLevel(float level) {
    /*!
     \param level Sets sustain level
     */
    sustainLevel = level;
    decayBase = (sustainLevel - targetRatioDR) * (1.0 - decayCoef);
   // //std::coutt << sustainLevel << std::endl;;
}

void ADSR::setTargetRatioA(float targetRatio)
///Sets exponential rate of attack
{
    ...
    attackBase = (1.0 + targetRatioA) * (1.0 - attackCoef);
    ////std::coutt << targetRatioA << "RatioA" << std::endl;
}

void ADSR::setTargetRatioDR(float targetRatio)
///set exponential rate of decay and release
{
    if (targetRatio < 0.000000001)
        targetRatio = 0.000000001;  // -180 dB
